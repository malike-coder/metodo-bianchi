 ...
 */
export async function analyzeRoomPhotoWithAi(photoFile: File, roomName: string): Promise<RoomAiAnalysis | null> {
  const apiKey = getGeminiApiKey();
  if (!apiKey) {
    console.log('[AI Service] No API Key set. Falling back to local mock scan.');
    return null;
  }
  ...
  try {
    ...
    const filePart = await fileToGenerativePart(photoFile);
    const prompt = `Analiza la siguiente fotografía de un ambiente doméstico (${roomName}) con el ojo de un neuroarquitecto y especialista en diseño de bienestar del Método Bianchi®.
...
export async function generateClientNarrativeWithAi(
  ...
  try {
    ...
    // Using gemini-1.5-pro as it handles complex instructions and JSON structure best
    const model = genAI.getGenerativeModel({
